...
if __name__ == '__main__':
    ...
    frames = int(config['model']['frames'])
    pred_itv = int(config['model']['pred_itv'])

    lm = Log_Manager(log_dir, name)
    dm = Data_Manager(data_dir, lm, batch_size, reshape_type=None, frames=frames, pred_itv=pred_itv)
    din, dout = dm.get_dims()
    model = DNN(log_dir, name, lr, dm, lm, din, dout)
